...
<!-- <svelte:window on:scroll={lockScroll} /> -->
{#each newAwards as { title, description, icon, cert }, index}
	<div class="card shadow-lg compact side bg-base-100">
		<div class="flex-row items-center space-x-4 card-body">
			...
			<div>
				<h2 class="card-title text-{colors(index)}">{title}</h2>
				{#if !cert}
				...
				{:else}
					...
					<label
						...
						class="btn btn-outline btn-{colors(
							index
						)} btn-sm modal-button">View Certificate</label
					>
					<!-- Modal toggle -->
					<input
					...
					/>
					<!-- Modal -->
					<div class="modal" on:click={leaveModal} bind:this={modal}>
